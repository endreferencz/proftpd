...
static int handle_lock_error(db_conn_t *conn, unsigned int nattempts) {
...
MODRET cmd_insert(cmd_rec *cmd) {
   ...
   */
  nattempts++;
  res = mysql_real_query(conn->mysql, query, strlen(query));
  while (res != 0) {
    if (handle_lock_error(conn, nattempts) == 0) {
      nattempts++;
      res = mysql_real_query(conn->mysql, query, strlen(query));
      continue;
    }
